...
uint32_t PacketSizeDistribution(void) {
  ...
  /*
    ...
    std::cout << p.size() << "," << avg_100/p.size() << "," << avg_300/p.size()
    << std::endl;
...
void UdpClient::Send(void) {
  ...
  Ptr<UniformRandomVariable> packetSize_index =
      CreateObject<UniformRandomVariable>();

  if (EnableTX[nodeId - 1]) // Not all nodes are enabled to transmit (see SUMO
                            // simulation details)
  {
    SeqTsHeader seqTs;    // Packet header for UDP Client/Server application
    seqTs.SetSeq(m_sent); // The packet header must be sequentially increased.
                          // Use the packet number m_sent
    ...
    v2xTag.SetTrafficType(
        VehicleTrafficType[nodeId -
                           1]); // Coexistence of periodic and aperiodic traffic
    v2xTag.SetPPPP(0x00);       // PPPP for CAM
    v2xTag.SetPrsvp(
        (double)(1000 * m_interval.GetSeconds())); // the required PHY
                                                   // reservation interval. For
                                                   // periodic traffic is ok
    v2xTag.SetNodeId(
        (uint32_t)
            nodeId); // Encapsulate the nodeId. It will be used at MAC layer
    v2xTag.SetReselectionCounter(
        (uint16_t)10000); // safe value for standard-compliant Cresel assignment
    ...
    if (VehicleTrafficType[nodeId - 1] == 0x01) // Aperiodic traffic
    {
      if (ETSITraffic) {
        ...
        NS_LOG_UNCOND("Udp node "
                      << nodeId << ": transmitting packet with size: " << m_size
                      << " and reserving resources using " << ReservationSize);
      ...
      } else // Exponential model
      {
        ...
        if (nodeId % ModuloSplit <= 1) {
          ...
          NS_LOG_INFO("Aperiodic inter-arrival time constant = "
                      << Aperiodic_Tgen_c[nodeId - 1]
                      << ", Exp mean = " << RndExp->GetMean());
        } else {
          T_gen = Aperiodic_Tgen_c[nodeId - 1] + RndExp_1->GetValue();
          NS_LOG_INFO("Aperiodic inter-arrival time constant = "
                      << Aperiodic_Tgen_c[nodeId - 1]
                      << ", Exp mean = " << RndExp_1->GetMean());
        }
        //        v2xTag.SetPdb ((double)Aperiodic_Tgen_c[nodeId-1]); // @LUCA
        //        modified later
        v2xTag.SetPdb(
            (double)PDB_Aperiodic[nodeId - 1]); // @LUCA modified later

        m_size = AperiodicPKTs_Size[packetSize_index->GetInteger(
            0, AperiodicPKTs_Size.size() - 1)];
        ...
        NS_LOG_UNCOND("Udp: UE "
                      << nodeId << " transmitting packet with size: "
                      << m_size + 35 << " B and reserving resources using "
                      << ReservationSize + 35 << "B. RRI " << v2xTag.GetPrsvp()
                      << " ms. Next packet in " << T_gen << " ms");
      }
      // std::cin.get();
    ...
    } else // Periodic traffic
    {
      ...
      //      m_size =
      //      AperiodicPKTs_Size[packetSize_index->GetInteger(0,AperiodicPKTs_Size.size()-1)];
    ...
    if ((VehicleTrafficType[nodeId - 1] == 0x01) && (ETSITraffic) &&
      ...
      CAMdebug << packetID << "," << Simulator::Now().GetSeconds() << ","
               << nodeId << "," << Pattern_index[nodeId - 1] << ","
               << CAMtraces[nodeId][Pattern_index[nodeId - 1]].first << ","
               << CAMtraces[nodeId][Pattern_index[nodeId - 1]].second << "\r\n";
      CAMdebug.close();
    }
    ...
    if (insideTX && enableUDPfiles) {
      ...
      filetest << packetID << "," << Simulator::Now().GetSeconds() << ","
               << nodeId << "," << xPosition << "," << yPosition << ","
               << (int)v2xTag.GetMessageType() << ","
               << (int)v2xTag.GetTrafficType() << "," << m_size + 34 << "\r\n";
      filetest.close();
    }
    ...
    // node by a SocketFactory specified by TypeId
    Ptr<Socket> sendSocket = Socket::CreateSocket(currentNode, UDPtid);
    sendSocket->SetAllowBroadcast(
        true); // Configure whether broadcast transmissions are allowed
    Ipv4Address destAddress =
        groupAddress; // Important! The SlTft will check for the groupAddress
                      // and forward the packet on the Sidelink bearer
    ...
    if (Ipv4Address::IsMatchingType(destAddress) == true) {
      sendSocket->Bind();
      sendSocket->Connect(
          InetSocketAddress(Ipv4Address::ConvertFrom(destAddress), destPort));
      // sendSocket->ShutdownRecv();
    } else if (Ipv6Address::IsMatchingType(destAddress) == true) {
      sendSocket->Bind6();
      sendSocket->Connect(
          Inet6SocketAddress(Ipv6Address::ConvertFrom(destAddress), destPort));
    }
    // Send data on the socket and check if returns an error or not (if -> Send
    // fails it returns -1)
    if ((sendSocket->Send(p)) >= 0) {
      ++m_sent;
      // NS_LOG_INFO ("TraceDelay TX " << m_size << " bytes to " <<
      // peerAddressStringStream.str () << " Uid: " << p->GetUid () << " Time: "
    ...
    if (m_sent < m_count) {
      if (VehicleTrafficType[nodeId - 1] ==
          0x00) //--- If it's PERIODIC traffic ---
      {
        Pattern_index[nodeId - 1]++;
        //         m_sendEvent = Simulator::Schedule (m_interval,
        //         &UdpClient::Send, this);
        m_sendEvent =
            Simulator::Schedule(MilliSeconds(T_gen), &UdpClient::Send, this);
    ...
    m_sendEvent =
        Simulator::Schedule(MilliSeconds(TrepPrint), &UdpClient::Send, this);
  }
}
...
void PacketSink::HandleRead(Ptr<Socket> socket) {
  ...
  while ((packet = socket->RecvFrom(from))) {
    ...
    if (packet->FindFirstMatchingByteTag(
            rxV2xTag)) // If the packet tag exists, inspect its content
    {
      ...
      rxPosX = currentPos.x;
      rxPosY = currentPos.y;
      TxRxDistance = std::sqrt(std::pow(genPosX - rxPosX, 2) +
                               std::pow(genPosY - rxPosY, 2));
    ...
    if (insideRX && enableUDPfiles) {
      ...
      filetest << rxPacketID << "," << tGenSec << ","
               << Simulator::Now().GetSeconds() << ","
               << Simulator::Now().GetSeconds() - tGenSec << "," << nodeId
               << "," << packet->GetSize() << "," << TxRxDistance << ","
               << numHops << "," << (int)messageType << ","
               << (int)rxV2xTag.GetTrafficType() << "," << (int)alreadyReceived
               << "\r\n";
      filetest.close();
    }
  }
...
void LoadCAMtraces(NodeContainer VehicleUEs) {
  ...
  for (NodeContainer::Iterator L = VehicleUEs.Begin(); L != VehicleUEs.End();
       ++L) {
    Ptr<Node> node = *L;
    ID = node->GetId();
    ...
    CAMtraceFile.open("/home/luca/Desktop/C-V2V/src/MoReV2X/CAM-tools/"
                      "CAM-model/CAMtraces/CAMtrace_" +
...
void updateNodePositions(std::ifstream *traceFile, NodeContainer ueResponders) {
  for (NodeContainer::Iterator L = ueResponders.Begin();
       L != ueResponders.End(); ++L) {
    Ptr<Node> node = *L;
    Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
    Ptr<ConstantPositionMobilityModel> velMob =
        DynamicCast<ConstantPositionMobilityModel>(mob);
    if (!velMob)
      continue; // Skip if not a ConstantPositionMobilityModel

    Vector newPos =
        getNextCoords(*traceFile); // Get next coordinates from the file
...
void Print(NodeContainer VehicleUEs) {
  ...
  for (NodeContainer::Iterator L = VehicleUEs.Begin(); L != VehicleUEs.End();
    ...
    if (inside) {
      positFile << Simulator::Now().GetSeconds() << "," << ID << "," << pos.x
                << "," << pos.y << "," << pos.z << "," << VelX[ID - 1] << ","
                << VelY[ID - 1] << "," << VelZ[ID - 1] << ","
                << (int)VehicleTrafficType[ID - 1] << ","
                << "1"
                << "\r\n";
    } else
      positFile << Simulator::Now().GetSeconds() << "," << ID << "," << pos.x
  ...
  if (traceFile.is_open())
  Simulator::Schedule(MilliSeconds(TrepPrint), &updateNodePositions, &traceFile,
                      VehicleUEs);
  positFile.close();
}
...
int main(int argc, char *argv[]) {
  ...
  //  AperiodicPKTs_Size = packetSize;  // Can be set with a different dimension
  //  too.
  ...
  bool randomV2VSelection =
      false; // If true, transmission resources are randomly selected
  ...
  uint32_t runNumber =
      1; // this is the default run --> this will be overridden shortly...
  ...
  std::vector<BoundingBox> BBs = {
      {533, 740, 830, 1027},    {1784, 1938, 1602, 1746},
      {2544, 2732, 2654, 2775}, {2561, 2689, 2989, 3080},
      {2278, 2389, 3592, 3716}, {1929, 2211, 3750, 3906},
      {1098, 1223, 3456, 3543}, {525, 618, 2931, 2993},
      {400, 541, 2171, 2293}};

  Point polygonTX[] = {{975, 1870},  {1540, 1626}, {1965, 2121},
                       {2556, 3253}, {1798, 3597}, {966, 2492}};
  Point polygonRX[] = {{962, 1861},  {1541, 1614}, {1975, 2114},
                       {2572, 3258}, {1793, 3609}, {953, 2491}};
  ...
  //  cmd.AddValue ("packetSize", "The Application-layer Packet Size in Byte",
  //  packetSize);
  ...
  cmd.AddValue("randomV2VSelection",
               "Whether V2V resources are randomly selected in autonomous "
               "scheduling mode",
               randomV2VSelection);
  cmd.AddValue("useRxCresel",
               "Mode 2: use the receiver SCI reselection counter", RxCresel);
  ...
  cmd.AddValue("VariableSize", "Generate variable size packets",
               VariablePacketSize);

  cmd.AddValue("Dynamic", "Enable Mode 2 Dynamic Scheduling",
               DynamicSchedulingMode2);
  cmd.AddValue("Adaptive",
               "Enable Mode 2 Adaptive Scheduling (mixed traffic only)",
               AdaptiveSchedulingMode2);
  cmd.AddValue("ReTx", "Allow re-transmissions (blind)", ReTranmissions);
  cmd.AddValue("ReEvaluation", "Allow re-evaluation of selected resources",
               ReEvaluation);
  cmd.AddValue("AllSlots", "Perform re-evaluation in all-slots",
               AllSlots_ReEvaluation);
  cmd.AddValue("UMH", "Perform re-evaluation only on re-transmissions",
               UMH_ReEvaluation);
  ...
  cmd.AddValue("Periodic", "Enable periodic traffic generation",
               PeriodicTraffic);
  cmd.AddValue("Aperiodic", "Enable aperiodic traffic generation",
               AperiodicTraffic);
  cmd.AddValue("Mixed",
               "Enable the mixed periodic/aperiodic traffic generation",
               MixedTraffic);
  cmd.AddValue("Percentage", "In mixed mode, the percentage of periodic UEs",
               PeriodicPercentage);

  cmd.AddValue("ETSI", "Enable the ETSI-Algorithm for the CAMs generation",
               ETSITraffic);

  cmd.AddValue(
      "AvgRRI",
      "Reserve resources with average RRI in case of aperiodic traffic",
      avgRRI);

  cmd.AddValue("FreqReuse", "Enable the frequency-reuse strategy",
               FrequencyReuse);
  ...
  if (SCS_factor.find(OFDM_numerology) == SCS_factor.end())
    ...
    NS_LOG_UNCOND("Adopted OFDM numerology is "
                  << (uint32_t)OFDM_numerology << ", SCS = "
                  << (15 * SCS_factor[OFDM_numerology]) << " kHz");
  ...
  NS_ASSERT_MSG(channelBW_RBs >= subchannelSize,
                "Channel bandwidth must be larger than the subchannel size");

  NS_LOG_UNCOND("Channel BW = "
                << channelBW << " MHz. Channel BW = " << channelBW_RBs
                << " RBs. Subchannel size = " << subchannelSize << " RBs");

  // RefSensitivity = GetRefSensitivity(15*SCS_factor[OFDM_numerology],
  // channelBW);
  ...
  NS_LOG_UNCOND("UE reference sensitivity = " << RefSensitivity
                                              << " dBm, RSSI threshold = "
                                              << CBR_RSSIthreshold << " dBm");

  std::map<uint16_t, std::vector<std::pair<double, double>>> SubchannelIndexMap;
  uint32_t NumGeoCells;

  if (FrequencyReuse) {
    NS_ASSERT_MSG(ReuseDistance != 0, "Reuse distance must be larger than "
                                      "zero. Set it using --ReuseDist option");
    GeoCellSize = GetGeoCellSize(subchannelSize, channelBW_RBs, ReuseDistance,
                                 &NumGeoCells);
    NS_LOG_UNCOND("Reuse distance "
                  << ReuseDistance << " meters. Geo-cell size " << GeoCellSize
                  << " meters, number of geo-cells: " << NumGeoCells);
    // Number of geo-cells coincides with the number of subchannels
    double X_coord = 0.0;
    uint32_t subchannel_index = 0;
    while (X_coord <= highwayLength) {
      NS_LOG_DEBUG("Assigning subchannel index "
                   << subchannel_index % NumGeoCells << " to geo-cell ["
                   << X_coord << "," << X_coord + GeoCellSize << ")");
      std::map<uint16_t, std::vector<std::pair<double, double>>>::iterator
          geoIT = SubchannelIndexMap.find(subchannel_index % NumGeoCells);
      if (geoIT != SubchannelIndexMap.end()) {
        SubchannelIndexMap[subchannel_index % NumGeoCells].push_back(
            std::pair<double, double>(X_coord, X_coord + GeoCellSize));
      } else {
        std::pair<double, double> tmpPair(X_coord, X_coord + GeoCellSize);
        std::vector<std::pair<double, double>> tmpVector = {tmpPair};
        SubchannelIndexMap.insert(
            std::pair<uint16_t, std::vector<std::pair<double, double>>>(
                subchannel_index % NumGeoCells, tmpVector));
      }
      X_coord += GeoCellSize;
      subchannel_index++;
    }
  ...
  for (uint16_t i=1; i< 20; i++)
  {
    ...
    NRamc->GetSlSubchAndTbSizeFromMcs(i*100-3, mcs, subchannelSize,
  channelBW_RBs, &TBlen_subCH, &TBlen_RBs); NS_LOG_UNCOND("Pkt size = " << i*100
  << " B requires " << TBlen_RBs << " RBs. Subchannel length = " << TBlen_subCH
  << " RBs");
  }
  std::cin.get();
  ...
  {
    uint16_t TBlen_subCH, TBlen_RBs;
    NRamc->GetSlSubchAndTbSizeFromMcs(200+i*200, mcs, subchannelSize,
  channelBW_RBs, &TBlen_subCH, &TBlen_RBs); NS_LOG_UNCOND("Pkt size = " <<
  200+i*200 << ", RBs " << TBlen_RBs << " Subchannels = " << TBlen_subCH);
  }

  uint16_t TBlen_subCH, TBlen_RBs;
  NRamc->GetSlSubchAndTbSizeFromMcs(156+34, mcs, subchannelSize, channelBW_RBs,
  &TBlen_subCH, &TBlen_RBs); NS_LOG_UNCOND("Pkt size = " << 190 << ", RBs " <<
  TBlen_RBs << " Subchannels = " << TBlen_subCH);
  NRamc->GetSlSubchAndTbSizeFromMcs(266+34, mcs, subchannelSize, channelBW_RBs,
  &TBlen_subCH, &TBlen_RBs); NS_LOG_UNCOND("Pkt size = " << 300 << ", RBs " <<
  TBlen_RBs << " Subchannels = " << TBlen_subCH); std::cin.get();*/
  ...
  if (ETSITraffic) {
    ...
    NS_ASSERT_MSG((PeriodicPercentage == 10) || (PeriodicPercentage == 25) ||
                      (PeriodicPercentage == 50) ||
                      (PeriodicPercentage == 75) || (PeriodicPercentage == 90),
                  "Non-valid periodic traffic percentage. Allowed values are "
                  "25, 50 and 75%");
  ...
  if (AdaptiveSchedulingMode2) {
    NS_ASSERT_MSG(
        MixedTraffic,
        "Adaptive scheduling is allowed only in mixed traffic scenarios");
    NS_ASSERT_MSG(!DynamicSchedulingMode2,
                  "With mixed traffic only dynamic or adaptive scheduling is "
                  "allowed, not both");
  }

  // NS_ASSERT_MSG (!(ExponentialModel and CAMtraceModel), "Choose only one
  // aperiodic traffic model");
  //  NS_ASSERT_MSG (!(ML_CAMtrace and GT_CAMtrace), "Choose the Machine
  //  Learning algorithm OR the Ground-Truth predictions"); // Da rimuovere per
  //  WNS3??
  ...
  if (PeriodicTraffic)
    outputPath = "results/Periodic_Dynamic" +
                 std::to_string(DynamicSchedulingMode2) + "_avgRRI" +
                 std::to_string(avgRRI) + "_VariableSize" +
                 std::to_string(VariablePacketSize) + "_ReEval" +
                 std::to_string(ReEvaluation) + "_" + std::to_string(ueCount) +
                 "_PDB" + std::to_string(inputPDB) + "_" +
                 std::to_string(seed) + "_" + std::to_string(runNumber) + "/";
  else if (AperiodicTraffic)
    outputPath = "results/Aperiodic_Dynamic" +
                 std::to_string(DynamicSchedulingMode2) + "_avgRRI" +
                 std::to_string(avgRRI) + "_VariableSize" +
                 std::to_string(VariablePacketSize) + "_ReEval" +
                 std::to_string(ReEvaluation) + "_" + std::to_string(ueCount) +
                 "_PDB" + std::to_string(inputPDB) + "_" +
                 std::to_string(seed) + "_" + std::to_string(runNumber) + "/";
  else if (AdaptiveSchedulingMode2)
    outputPath = "results/MixedAdaptive_Dynamic" +
                 std::to_string(DynamicSchedulingMode2) + "_avgRRI" +
                 std::to_string(avgRRI) + "_VariableSize" +
                 std::to_string(VariablePacketSize) + "_ReEval" +
                 std::to_string(ReEvaluation) + "_" + std::to_string(ueCount) +
                 "_PDB" + std::to_string(inputPDB) + "_Perc" +
                 std::to_string(PeriodicPercentage) + "_" +
                 std::to_string(seed) + "_" + std::to_string(runNumber) + "/";
  else
    outputPath = "results/Mixed_Dynamic" +
                 std::to_string(DynamicSchedulingMode2) + "_avgRRI" +
                 std::to_string(avgRRI) + "_VariableSize" +
                 std::to_string(VariablePacketSize) + "_ReEval" +
                 std::to_string(ReEvaluation) + "_" + std::to_string(ueCount) +
  ...
  /*
    if (PeriodicTraffic)
      outputPath = "results/Periodic_Dynamic" +
    std::to_string(DynamicSchedulingMode2) + "_FreqReuse" +
    std::to_string(FrequencyReuse) + "_" + std::to_string(channelBW) + "_" +
    std::to_string(ReuseDistance) + "_" + std::to_string(ueCount) + "_" +
    std::to_string(seed) + "_" + std::to_string(runNumber) + "/"; else if
    (AperiodicTraffic) outputPath = "results/Aperiodic_Dynamic" +
    std::to_string(DynamicSchedulingMode2) + "_FreqReuse" +
    std::to_string(FrequencyReuse) + "_" + std::to_string(channelBW) + "_" +
    std::to_string(ReuseDistance) + "_" + std::to_string(ueCount) + "_" +
    std::to_string(seed) + "_" + std::to_string(runNumber) + "/"; else
      outputPath = "results/sidelink_" + std::to_string(ueCount) + "_" +
    std::to_string(seed) + "_" + std::to_string(runNumber) + "/";
  */
  ...
  readme << " - Channel BW = " << channelBW
         << " MHz. Channel BW = " << channelBW_RBs << " RBs" << std::endl;
  ...
  if (MixedTraffic) {
    readme << " --- Periodic users = " << PeriodicPercentage << "%"
           << std::endl;
    readme << " --- Adaptive scheduling " << AdaptiveSchedulingMode2
           << std::endl;
  }
  ...
  if (FrequencyReuse) {
    ...
    readme << " ----- Reuse distance = " << ReuseDistance << " meters"
           << std::endl;
    readme << " ----- Geo-cell size = " << GeoCellSize << " meters"
           << std::endl;
    readme << " ----- Geo-cells number = " << NumGeoCells << std::endl;
  } else
    readme << " --- Frequency-reuse disabled " << std::endl;
  readme << "- traceFile = " << traceFileName << std::endl;
  readme.close();
  ...
  if (FrequencyReuse) {
    ...
    for (std::map<uint16_t, std::vector<std::pair<double, double>>>::iterator
             mapIT = SubchannelIndexMap.begin();
         mapIT != SubchannelIndexMap.end(); mapIT++) {
      //    NS_LOG_UNCOND("Subchannel index " << mapIT->first << " includes the
      //    following geo-cells:");
      std::vector<std::pair<double, double>> GeoCellsVector = mapIT->second;
      for (std::vector<std::pair<double, double>>::iterator GeoCellsIT =
               GeoCellsVector.begin();
           GeoCellsIT != GeoCellsVector.end(); GeoCellsIT++) {
        //        NS_LOG_UNCOND("--- [" << GeoCellsIT->first << "," <<
        //        GeoCellsIT->second << ")");
        FreqReuseMap << GeoCellsIT->first << "," << GeoCellsIT->second << ","
  ...
  PositionChecker.initPolygon(polygonTX,
                              (int)sizeof(polygonTX) / sizeof(polygonTX[0]),
                              "TX"); // Filter TX users
  PositionChecker.initPolygon(polygonRX,
                              (int)sizeof(polygonRX) / sizeof(polygonRX[0]),
                              "RX"); // Filter RX users
  ...
  Config::SetDefault(
      "ns3::NrV2XUeMac::SlGrantMcs",
      UintegerValue(
          mcs)); // The MCS of the SL grant, must be [0..15] (default 0)
  Config::SetDefault("ns3::NrV2XUeMac::ListL2Enabled", BooleanValue(false));

  Config::SetDefault("ns3::NrV2XUeMac::MixedTraffic",
                     BooleanValue(MixedTraffic));
  Config::SetDefault("ns3::NrV2XUeMac::AllowReEvaluation",
                     BooleanValue(ReEvaluation));
  Config::SetDefault("ns3::NrV2XUeMac::AllSlotsReEvaluation",
                     BooleanValue(AllSlots_ReEvaluation));
  Config::SetDefault("ns3::NrV2XUeMac::RSRPthreshold",
                     DoubleValue(MAC_RSRPthreshold));
  Config::SetDefault("ns3::NrV2XUeMac::EnableReTx",
                     BooleanValue(ReTranmissions));
  Config::SetDefault("ns3::NrV2XUeMac::DynamicScheduling",
                     BooleanValue(DynamicSchedulingMode2));
  Config::SetDefault("ns3::NrV2XUeMac::AdaptiveScheduling",
                     BooleanValue(AdaptiveSchedulingMode2));
  Config::SetDefault("ns3::NrV2XUeMac::UMHReEvaluation",
                     BooleanValue(UMH_ReEvaluation));
  Config::SetDefault("ns3::NrV2XUeMac::FrequencyReuse",
                     BooleanValue(FrequencyReuse));
  ...
  Config::SetDefault("ns3::NistLteUePowerControl::PoNominalPusch",
                     IntegerValue(-106));
  Config::SetDefault(
      "ns3::NistLteUePowerControl::PscchTxPower",
      DoubleValue(
          ueTxPower)); // Setting the Transmission Power for the control channel
  Config::SetDefault("ns3::NistLteUePowerControl::PsschTxPower",
                     DoubleValue(ueTxPower)); // Setting the Transmission Power
                                              // for the sidelink channel
  Config::SetDefault("ns3::NrV2XUePhy::RsrpUeMeasThreshold",
                     DoubleValue(-10.0)); // Setting the RSRP threshold
  Config::SetDefault("ns3::NrV2XUePhy::ReferenceSensitivity",
                     DoubleValue(RefSensitivity));
  Config::SetDefault("ns3::NrV2XUePhy::RSSIthreshold",
                     DoubleValue(CBR_RSSIthreshold));

  // Configure spectrum layer
  Config::SetDefault("ns3::NrV2XSpectrumPhy::ReferenceSensitivity",
                     DoubleValue(RefSensitivity));
  Config::SetDefault("ns3::NrV2XSpectrumPhy::CtrlErrorModelEnabled",
                     BooleanValue(CtrlErrorModelEnabled)); // Set but not used
  Config::SetDefault("ns3::NrV2XSpectrumPhy::CtrlFullDuplexEnabled",
                     BooleanValue(!CtrlErrorModelEnabled)); // Set but not used
  Config::SetDefault(
      "ns3::NrV2XSpectrumPhy::SaveCollisionLossesUnimore",
      BooleanValue(true)); // fare var apposta   // Enable the collision and
                           // propagation loss event saving

  // Used for
  Config::SetDefault("ns3::NrV2XUeMac::RandomV2VSelection",
                     BooleanValue(randomV2VSelection));

  NS_LOG_INFO("Starting network configuration...");

  Config::SetDefault("ns3::NistLteRlcUm::MaxTxBufferSize",
                     StringValue("100000"));
  Config::SetDefault(
      "ns3::DropTailQueue::MaxPackets",
      UintegerValue(10000000)); // In order not to run out of packets ;)

  // Configure the path for storing output
  // files--------------------------------------------------------------------
  ...
  Config::SetDefault("ns3::NrV2XSpectrumPhy::OutputPath",
                     StringValue(outputPath));
  // Configure the saving
  // period------------------------------------------------------------------------------------
  // Tradeoff between speed of the simulation and memory requirements
  double SavingPeriod = 2.0;
  Config::SetDefault("ns3::NrV2XUeMac::SavingPeriod",
                     DoubleValue(SavingPeriod));
  Config::SetDefault("ns3::NrV2XUePhy::SavingPeriod",
                     DoubleValue(SavingPeriod));
  Config::SetDefault("ns3::NrV2XSpectrumPhy::SavingPeriod",
                     DoubleValue(SavingPeriod));
  //-------------------------------------------------------------------------------------------------------

  // Configuration of NR-V2X
  // numerologies--------------------------------------------------------------------
  Config::SetDefault("ns3::NrV2XUeMac::SubchannelSize",
                     UintegerValue(subchannelSize));
  Config::SetDefault("ns3::NrV2XUeMac::RBsBandwidth",
                     UintegerValue(channelBW_RBs));
  Config::SetDefault("ns3::NrV2XUeMac::SlotDuration",
                     DoubleValue(1.0 / SCS_factor[OFDM_numerology]));
  Config::SetDefault("ns3::NrV2XUeMac::NumerologyIndex",
                     UintegerValue(OFDM_numerology));

  Config::SetDefault("ns3::NrV2XSpectrumPhy::SubchannelSize",
                     UintegerValue(subchannelSize));
  Config::SetDefault("ns3::NrV2XSpectrumPhy::RBsBandwidth",
                     UintegerValue(channelBW_RBs));
  ...
  case 0:
    ...
    Config::SetDefault("ns3::NrV2XSpectrumPhy::SubCarrierSpacing",
                       UintegerValue(15));
  ...
  case 3:
    Config::SetDefault("ns3::NrV2XUePhy::SidelinkDataDuration",
                       TimeValue(NanoSeconds(0.125e6 - 8920 - 1)));
    Config::SetDefault("ns3::NrV2XSpectrumPhy::SubCarrierSpacing",
                       UintegerValue(120));
    Config::SetDefault("ns3::NrV2XUePhy::SubCarrierSpacing",
                       UintegerValue(120));
    break;
  };
  ...
  Config::SetDefault("ns3::NrV2XUePhy::SlotDuration",
                     DoubleValue(1.0 / SCS_factor[OFDM_numerology]));
  Config::SetDefault("ns3::NrV2XUePhy::SubchannelSize",
                     UintegerValue(subchannelSize));
  Config::SetDefault("ns3::NrV2XUePhy::RBsBandwidth",
                     UintegerValue(channelBW_RBs));
  ...
  // Config::SetDefault ("ns3::NrV2XAmc::PSSCH_DMRStimePattern", StringValue
  // ("{2,3,4}"));

  //-------------------------------------------------------------------------------------------------------

  // These two lines are not used. Needed only to configure the simulation using
  // a stored input configuration file
  ConfigStore inputConfig;
  inputConfig.ConfigureDefaults();

  NS_LOG_INFO("Creating helpers...");
  //@DOC Not really needed: ns3 must set up the LTE infrastructure to run the
  //simulation (at least the PDN-GW or PGW to assign IP addresses)
  Ptr<NistPointToPointEpcHelper> epcHelper =
      CreateObject<NistPointToPointEpcHelper>(); // NIST EPC helper, used later
  Ptr<Node> pgw = epcHelper->GetPgwNode();

  Ptr<NistLteHelper> lteHelper =
      CreateObject<NistLteHelper>(); // Important! It's the NIST modified
                                     // version of a lteHelper, which is needed
                                     // to create and configure LTE network
                                     // entities (see online Doc)

  //@NEW
  //@DOC: now setting the operating frequency [in GHz]
  // Set 3gpp pathloss model
  Config::SetDefault("ns3::NrV2XPropagationLossModel::Frequency",
                     DoubleValue(5.9));
  Config::SetDefault("ns3::NrV2XPropagationLossModel::Sigma", DoubleValue(3.0));
  Config::SetDefault("ns3::NrV2XPropagationLossModel::SigmaNLOSv",
                     DoubleValue(4.0));
  Config::SetDefault("ns3::NrV2XPropagationLossModel::DecorrDistance",
                     DoubleValue(25.0));
  ...
  lteHelper->SetAttribute(
      "UseSidelink",
      BooleanValue(true)); // ObjectBase::SetAttribute() sets a single
                           // attribute, raising fatal errors if unsuccessful
  lteHelper->SetEpcHelper(epcHelper); // Set the NIST ECP helper as lteHelper

  lteHelper->Initialize(); // Invoke DoInitialize() on all Objects aggregated to
                           // this one
  ...
  for (uint32_t t = 0; t < ueCount; ++t) {
    ...
    // LTENodeState is a new class developed from scratch. Defined before the
    // main()
    Ptr<LTENodeState> nodeState = CreateObject<LTENodeState>();
    nodeState->SetNode(lteNode);
    lteNode->AggregateObject(
        nodeState); // Aggregate two objects together. Now it's possible to call
                    // GetObject() on one to get the other and vice-versa.
  ...
  for (NodeContainer::Iterator L = ueResponders.Begin();
    ...
    //    double yPos = laneNumber->GetInteger(1,1)*laneWidth; // 1 lane
    //    simulation
  ...
  //   <ListPositionAllocator>(); positionAlloc ->Add(getNextCoords(traceFile));
  ...
  for (NodeContainer::Iterator L = ueResponders.Begin();
    ...
    if ((AperiodicTraffic) || (ETSITraffic))
      //      VehicleTrafficType[ID-1] = 0x01;  // 0x00 for periodic traffic.
      //      0x01 for aperiodic traffic
  ...
  if ((MixedTraffic) || (AperiodicTraffic) || (PeriodicTraffic)) {
    //@LUCA setting the random variable generator for working with aperiodic
    //traffic Tgen_aperiodic_c = 50;
    ...
    //     LargestAperiodicSize = 100; // Largest packet size for aperiodic
    //     traffic
    for (uint16_t k = 1; k <= LargestAperiodicSize / quantizationStep; k++) {
      if (VariablePacketSize)
        AperiodicPKTs_Size.push_back(k * quantizationStep - 35);
      else
        AperiodicPKTs_Size.push_back(
            200 - 35); // Valid packet sizes from 100 to 1000 bytes with 100
                       // bytes quantization step
    }
    ...
    //     PeriodicPKTs_Size = {190-34, 190-34, 190-34, 190-34 ,190-34};
    //     //Account for the overhead PeriodicPKTs_Size = {300-34, 190-34,
    //     190-34, 190-34 ,190-34}; //Account for the overhead PeriodicPKTs_Size
    //     = {300-34, 300-34, 300-34, 300-34 ,300-34}; //Account for the
    //     overhead
    PeriodicPKTs_Size = {200 - 35, 200 - 35, 200 - 35, 200 - 35,
                         200 - 35}; // Account for the overhead
    LargestPeriodicSize =
        PeriodicPKTs_Size[0]; // 300 bytes is the largest packet size for
                              // aperiodic traffic

    Ptr<UniformRandomVariable> random_index =
        CreateObject<UniformRandomVariable>();
    for (NodeContainer::Iterator NN = ueResponders.Begin();
         NN != ueResponders.End(); ++NN) // UEs are indexed starting from 1
    {
      Pattern_index.push_back(
          random_index->GetInteger(0, PeriodicPKTs_Size.size() - 1));
    }
  } else // It's ETSI traffic
  {
    for (NodeContainer::Iterator NN = ueResponders.Begin();
         NN != ueResponders.End(); ++NN) {
      Pattern_index.push_back(0);
    }
    LargestCAMSize = 850;
    //     system("CAM-tools/test.sh");

    system(("cd src/MoReV2X/CAM-tools/CAM-model/ && python3 "
            "NS3_traces_generation.py -p CAMtraces --model Complete --scenario "
            "Highway --profile Volkswagen -m 5 -n " +
            std::to_string(ueCount) + " -t 30 ")
               .c_str());
  ...
  // NetDeviceContainer ueSendersDevs = lteHelper->InstallUeDevice
  // (ueResponders); ueDevs.Add (ueSendersDevs);
  ...
  for (uint32_t u = 0; u < ueResponders.GetN();
       ++u) // Could have used an iterator also here. Anyway, the result is the
            // same
  {
    Ptr<Node> ueNode = ueResponders.Get(u);
    //      std::cout << ueNode -> GetObject<Ipv4> () ->
    //      GetAddress(1,0).GetLocal() << std::endl;
  ...
  // UDP application: in our case, this is the application that generates CAMs
  // and DENMs
  ...
  //  Ipv4AddressGenerator::Init(Ipv4Address ("225.0.0.0"), Ipv4Mask
  //  ("255.0.0.0")); groupAddress = Ipv4AddressGenerator::NextAddress (Ipv4Mask
  //  ("255.0.0.0"));
  groupAddress = "225.0.0.1";
  std::cout << "Group address " << groupAddress << std::endl;
  UdpClientHelper udpClient(
      groupAddress,
      8000); // set destination IP address and UDP port (8000 in this case). The
             // group address is used to set the Sidelink Bearers

  udpClient.SetAttribute("MaxPackets", UintegerValue(100000));
  udpClient.SetAttribute(
      "Interval",
      TimeValue(MilliSeconds(100))); // Useful only for periodic traffic
  udpClient.SetAttribute("PacketSize",
                         UintegerValue(423)); // Fake value, useless

  ApplicationContainer clientApps = udpClient.Install(ueResponders);
  //  ApplicationContainer clientApps = udpClient.Install(ueResponders.Get(0));
  //  // Similar to NodeContainer, but now is a vector of smart pointers
  //  pointing to applications
  //  clientApps.Add(udpClient.Install(ueResponders.Get(1)));
  Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable>();

  //@OLD  clientApps.Get(0) -> SetStartTime(Seconds (rand ->
  //GetValue(0.001, 1.0)));
  for (uint32_t i = 0; i < ueResponders.GetN(); i++) {
    ...
    clientApps.Get(i)->SetStopTime(
        Seconds(simTime + 0.5)); // The simulation ends at simTime + 1, so we
                                 // have a buffer of 0.5s
  }
  ...
  //  clientApps.Get(0) -> SetStopTime (Seconds (simTime + 0.5)); // The
  //  simulation ends at simTime + 1, so we have a buffer of 0.5s
  ...
  PacketSinkHelper clientPacketSinkHelper(
      "ns3::UdpSocketFactory",
      InetSocketAddress(
          Ipv4Address::GetAny(),
          8000)); // Ipv4Address::GetAny() returns the dummy 0.0.0.0 address.
                  // The port number must be the correct one
  ...
  // At this point, each V-UE mounts both a Client and a Server (Sink)
  // application
  ...
  Ptr<NistSlTft> tftRx =
      Create<NistSlTft>(NistSlTft::BIDIRECTIONAL, groupAddress, groupL2Address);
  proseHelper->ActivateSidelinkBearer(
      Seconds(0.001), ueDevs,
      tftRx); // Set accordingly with clientApps StartTime! 0.1 Seconds is the
              // ActivationTime of the Sidelink bearer
  ...
  preconfiguration.preconfigGeneral.slBandwidth =
      channelBW_RBs; // original: 50 PRBs = 10MHz
  preconfiguration.preconfigComm.nbPools =
      1; // the number of pools, not relevant for V2V
  ...
  std::cout << "bitmap=" << std::hex << pscchBitmapValue
            << '\n'; // this is the PSCCH subframe pool bitmap, from NIST D2D
                     // implementation
  ...
  pfactory.SetHaveUeSelectedResourceConfig(
      false); // Mode1 --> done in this way, it does not work
